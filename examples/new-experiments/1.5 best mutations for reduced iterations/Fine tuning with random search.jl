...
file = readlines(open("1.4 digest.txt", "r"))
combinations = [[parse(Float64, element) for element in (Vector{String}(split(split(strip(line), "│")[2]))[1:8])] for line in file]
...
@everywhere function run_evolution_experiment(params::Union{Vector{Float64}, Vector{Float64}})
    ...
    evolution_parameters = EvolutionParameters(
        ...
        UInt16(1),     # Min numbe r of random hidden layers
