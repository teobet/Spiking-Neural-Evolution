...
file = readlines(open("1.2 digest.txt", "r"))
combinations = [[parse(Float64, element) for element in (Vector{String}(split(split(strip(line), "│")[2]))[1:8])] for line in file[1:40]]
tuned_combinations = []
for combination in combinations
    for _ in 1:100
        new_layer = max(0.0, combination[1] + ((rand() * 0.03) - 0.015))  # Randomly adjust by up to ±0.015
        remove_layer = max(0.0, combination[2] + ((rand() * 0.03) - 0.015))
        new_neuron = max(0.0, combination[3] + ((rand() * 0.03) - 0.015))
        remove_neuron = max(0.0, combination[4] + ((rand() * 0.03) - 0.015))
        new_rule = max(0.0, combination[5] + ((rand() * 0.03) - 0.015))
        remove_rule = max(0.0, combination[6] + ((rand() * 0.03) - 0.015))
        new_input = max(0.0, combination[7] + ((rand() * 0.03) - 0.015))
        remove_input = max(0.0, combination[8] + ((rand() * 0.03) - 0.015))
