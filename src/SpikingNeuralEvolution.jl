module SpikingNeuralEvolution
    ...
    function Simulate(examples::Vector{Vector{Bool}}, labels::Vector{Vector{Bool}}, evolution_parameters::EvolutionParameters, mutation_probabilities::MutationProbabilities)
        ...
        # The population of circuits will contain a number of individual between the specified
        ...
        for i in 1:population
            random_network = GenerateRandomCircuit(
                inputs, 
                outputs, 
                min_hidden_layers, 
                max_hidden_layers
        ...
        best_circuit = simulations[1, :].Network # <=> simulations.Network[1]
   ...
   #TODO da commentare
   function Evolve(f::Function, inputs::UInt16, evolution_parameters::EvolutionParameters, mutation_parameters::MutationProbabilities, verbose::Bool = false)
        if inputs > 2^10
        ...
        for exec in 1:evolution_parameters.simulations
            ...
            if verbose 
                println("Execution $exec done (max fitness: " * string(executions[exec][2][end]) * ") in " * string(executions[exec][3]) * " iterations.") 
            end
        end
    ...
    function Evolve(f::Function, inputs::UInt16, verbose::Bool = false)
        return Evolve(
            ...
            MutationProbabilities(
                ...
                0.005,  # Add rule
                0.080,  # Remove rules
                0.010,   # Add input line
                0.010   # Remove input line
            ),
            verbose
        )
    end

    function Evolve(f::Function, inputs::Int, verbose::Bool = false)
        return Evolve(
            f, 
            UInt16(inputs),
            EvolutionParameters(
    ...
    function Evolve(f::Function, inputs::UInt16, evolution_parameters::EvolutionParameters, verbose::Bool = false)
        return Evolve(
            f, 
            inputs,
